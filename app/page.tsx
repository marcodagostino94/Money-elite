...
function TransactionModal({ kind, close, add, accounts, cards, categories, preset = "normal", defaultAccount, cardId, initial, editing = false, refundSource }: { kind: ActionKind; close: () => void; add: (t: Transaction) => void | Promise<void>; accounts: MoneyAccount[]; cards: MoneyCard[]; categories: MoneyCategory[]; preset?: "normal" | "planned" | "subscription"; defaultAccount?: string; cardId?: string; initial?: Transaction; editing?: boolean; refundSource?: Transaction }) {
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}>
    ...
    {categoryOpen && <div className="picker-dim-layer keyboard-aware-picker" onMouseDown={()=>setCategoryOpen(false)} onWheel={event=>event.stopPropagation()} onTouchMove={event=>event.stopPropagation()}><div className="category-picker" onMouseDown={event=>event.stopPropagation()}>
    ...
    {amountPadTarget&&<div className="money-keypad-layer" onPointerDown={event=>{if(event.target===event.currentTarget)setAmountPadTarget(null)}}><div className="money-keypad"><div className="money-keypad-head"><span>{amountPadTarget==="received"?destinationAccount?.currency:amountCurrency}</span><strong>{amountPadTarget==="received"?(receivedValue||"0,00"):(amountValue||"0,00")}</strong><button type="button" onPointerDown={event=>{event.preventDefault();pressAmountKey("backspace")}} aria-label="Cancella ultima cifra"><AppIcon name="back" size={22}/></button></div><div className="money-keypad-grid">{["1","2","3","4","5","6","7","8","9",",","0"].map(key=><button type="button" key={key} onPointerDown={event=>{event.preventDefault();pressAmountKey(key)}}>{key}</button>)}<button type="button" className={`money-keypad-confirm ${kind}`} onPointerDown={event=>{event.preventDefault();event.stopPropagation()}} onPointerUp={event=>{event.preventDefault();event.stopPropagation();setAmountPadTarget(null)}} onClick={event=>{event.preventDefault();event.stopPropagation()}} aria-label="Chiudi tastierino"><AppIcon name="check" size={28}/></button></div></div></div>}
  </form></div>
}
