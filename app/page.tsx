...
const netBudgetSpend = (transactions: Transaction[], categoryIds: Set<string>, monthKey: string) => {
  const periodRows = transactions.filter(transaction=>isEffectiveTransaction(transaction)&&transaction.dateISO?.startsWith(monthKey));
  const expenses = periodRows.filter(transaction=>transaction.kind==="expense"&&Boolean(transaction.categoryId&&categoryIds.has(transaction.categoryId)));
  const expenseIds = new Set(expenses.map(transaction=>transaction.id));
  const refunds = periodRows.filter(transaction=>(transaction.kind==="refund"||transaction.isRefund)&&(
    Boolean(transaction.categoryId&&categoryIds.has(transaction.categoryId)) || Boolean(transaction.refundOf&&expenseIds.has(transaction.refundOf))
  ));
  return Math.max(0,expenses.reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0)-refunds.reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0));
};
...
const currentCardCycleMonth = (card: MoneyCard, today = new Date()) => {
  if (card.periodType === "no_period") return monthKeyFromDate(today);
  const startDay = Math.max(1, Math.min(31, card.cycleStartDay ?? 1));
  return today.getDate() < startDay ? shiftMonthKey(monthKeyFromDate(today), -1) : monthKeyFromDate(today);
};
...
const iconMap: Record<string, L.LucideIcon> = {
  ...
  debts:L.HandCoins,report:L.ChartPie,category:L.Shapes,settings:L.Settings,notification:L.Bell,income:L.ArrowDownLeft,
...
function Dashboard({ transactions, accounts, cards, budgets, categories, recurrences, primaryCurrency, dashboardAccountIds, setActive, confirmTransaction, openTransaction }: { transactions: Transaction[]; accounts: MoneyAccount[]; cards: MoneyCard[]; budgets: MoneyBudget[]; categories: MoneyCategory[]; recurrences: MoneyRecurrence[]; primaryCurrency:string; dashboardAccountIds: string[]; setActive: (s: Section) => void; confirmTransaction: (t: Transaction) => void | Promise<void>; openTransaction: (t: Transaction) => void }) {
  ...
  const dashboardBudgets = budgets.filter(item=>item.month.startsWith(currentMonth)).map(item=>{const category=categories.find(c=>c.id===item.categoryId);const categoryIds=new Set([item.categoryId,...categories.filter(child=>child.parentId===item.categoryId).map(child=>child.id)]);const spent=netBudgetSpend(effectiveTransactions,categoryIds,currentMonth);return {name:category?.name||"Categoria",spent,limit:item.amount,color:category?.color||"#7c65b5"};});
  ...
  const futurePlanned = recurrences
    ...
    .slice(0,5);
  const recent = transactions.filter(item=>(!item.dueDate||item.confirmedAt)&&(!item.cardId||item.kind==="card_repayment")).slice(0,5);
...
function CreditCardsSection({ onAdd, cards, accounts, transactions, refresh, openTransaction }: { onAdd: (kind: ActionKind, defaultAccount?: string, cardId?: string) => void; cards: MoneyCard[]; accounts: MoneyAccount[]; transactions: Transaction[]; refresh: () => Promise<void>; openTransaction: (transaction: Transaction) => void }) {
  ...
  const archiveCard = async(card:MoneyCard) => {
    ...
    if(!window.confirm(`Estinguere ${card.name}? La carta verrà rimossa da quelle attive, ma tutte le operazioni storiche resteranno conservate.`))return;
    const {error}=await getSupabaseBrowserClient().from("cards").update({archived_at:new Date().toISOString()}).eq("id",card.id);
    if(error){window.alert(error.message);return;}
    setDetailId(null);setEditor(null);setActions(false);await refresh();
  };
...
function BudgetSection({ budgets, categories, transactions, refresh }: { budgets: MoneyBudget[]; categories: MoneyCategory[]; transactions: Transaction[]; refresh: () => Promise<void> }) {
  ...
  const details=visible.map(item=>{const category=categories.find(c=>c.id===item.categoryId);const categoryIds=new Set([item.categoryId,...categories.filter(child=>child.parentId===item.categoryId).map(child=>child.id)]);const spent=netBudgetSpend(transactions,categoryIds,month);return {item,category,spent};});
  ...
  if(selected){const categoryIds=new Set([selected.item.categoryId,...categories.filter(child=>child.parentId===selected.item.categoryId).map(child=>child.id)]);const periodRows=transactions.filter(transaction=>isEffectiveTransaction(transaction)&&transaction.dateISO?.startsWith(month));const expenseRows=periodRows.filter(transaction=>transaction.kind==="expense"&&Boolean(transaction.categoryId&&categoryIds.has(transaction.categoryId)));const expenseIds=new Set(expenseRows.map(transaction=>transaction.id));const refundRows=periodRows.filter(transaction=>(transaction.kind==="refund"||transaction.isRefund)&&(Boolean(transaction.categoryId&&categoryIds.has(transaction.categoryId))||Boolean(transaction.refundOf&&expenseIds.has(transaction.refundOf))));const rows=[...expenseRows,...refundRows].sort((a,b)=>(b.dateISO||"").localeCompare(a.dateISO||""));return <section className="section-page budget-detail-page"><div className="inner-page-header"><button onClick={()=>setDetailId(null)}><AppIcon name="back"/></button><div><small>BUDGET · {monthLabel(month).toUpperCase()}</small><h2>{selected.category?.name||"Categoria"}</h2><p>Uscite e rimborsi che compongono la spesa netta</p></div><button className="outline edit-card-button" onClick={()=>setEditor(selected.item)}><AppIcon name="edit" size={15}/> Modifica</button></div><div className="big-budget"><div><small>BUDGET</small><h2>{money(selected.item.amount)}</h2></div><div><small>SPESA NETTA</small><h2>{money(selected.spent)}</h2></div><div><small>DISPONIBILE</small><h2 className={selected.item.amount-selected.spent>=0?"positive":""}>{money(selected.item.amount-selected.spent)}</h2></div></div><article className="panel month-transactions">{rows.length?rows.map(transaction=><TransactionRow key={transaction.id} t={transaction}/>):<div className="empty">Nessuna transazione per questo budget nel mese selezionato.</div>}</article>{editor&&editor!=="new"&&<BudgetModal budget={editor} categories={categories} month={`${month}-01`} close={()=>setEditor(null)} refresh={refresh}/>}</section>}
  return <section className="section-page"><div className="budget-month-row"><label>Mese<select value={month} disabled><option>{new Intl.DateTimeFormat("it-IT",{month:"long",year:"numeric"}).format(new Date())}</option></select></label><button className="outline" onClick={()=>setEditor("new")}>＋ Crea budget</button></div><div className="big-budget"><div><small>BUDGET TOTALI</small><h2>{money(total)}</h2></div><div><small>SPESO</small><h2>{money(spent)}</h2></div><div><small>DISPONIBILE</small><h2 className="positive">{money(total-spent)}</h2></div></div><div className="item-grid">{details.length?details.map(({item,category,spent})=><article className="item-card budget-card budget-card-clickable" key={item.id} onClick={()=>setDetailId(item.id)}><div className="item-body"><small>BUDGET MENSILE</small><h3>{category?.name||"Categoria"}</h3><div className="progress"><i style={{width:`${Math.min(100,spent/item.amount*100)}%`,background:category?.color||"#7c65b5"}}/></div><strong>{money(spent)} <span>di {money(item.amount)}</span></strong></div><div className="budget-card-actions"><button onClick={event=>{event.stopPropagation();setEditor(item)}} aria-label="Modifica budget"><AppIcon name="edit"/></button><button onClick={event=>{event.stopPropagation();void remove(item.id)}} aria-label="Elimina budget"><AppIcon name="trash"/></button><button onClick={event=>{event.stopPropagation();setDetailId(item.id)}} aria-label="Vedi transazioni"><AppIcon name="list"/></button></div></article>):<div className="empty panel">Nessun budget per questo mese.</div>}</div>{editor&&<BudgetModal budget={editor==="new"?undefined:editor} categories={categories} month={`${month}-01`} close={()=>setEditor(null)} refresh={refresh}/>}</section>;
}

function BudgetModal({ budget, categories, month, close, refresh }: { budget?:MoneyBudget; categories: MoneyCategory[]; month: string; close: () => void; refresh: () => Promise<void> }) {
  const leaves=categories.filter(c=>c.kind==="expense"&&!categories.some(parent=>parent.parentId===c.id));
  const save=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const fd=new FormData(event.currentTarget);const {data:{user}}=await getSupabaseBrowserClient().auth.getUser();if(!user)return;const payload={user_id:user.id,category_id:String(fd.get("category")),amount:parseItalianAmount(fd.get("amount")),month};const {error}=budget?await getSupabaseBrowserClient().from("budgets").update(payload).eq("id",budget.id):await getSupabaseBrowserClient().from("budgets").insert(payload);if(error){alert(error.message);return;}await refresh();close();};
  return <div className="modal-backdrop"><form className="modal entity-modal" onSubmit={save}><div className="modal-title"><div><small>{budget?"MODIFICA BUDGET":"NUOVO BUDGET"}</small><h2>{budget?"Modifica budget":"Crea budget"}</h2></div></div><label>Categoria<select name="category" required defaultValue={budget?.categoryId}>{leaves.map(c=>{const parent=c.parentId?categories.find(p=>p.id===c.parentId):null;return <option key={c.id} value={c.id}>{parent?`${parent.name} › `:""}{c.name}</option>})}</select></label><label>Ammontare<input name="amount" type="text" inputMode="decimal" defaultValue={amountInput(budget?.amount)} placeholder="0,00" required/></label><div className="modal-actions"><button type="button" className="cancel" onClick={close}>Annulla</button><button className="save-action transfer">{budget?"Salva modifiche":"Salva"}</button></div></form></div>;
}
...
function ReportSectionInteractive({transactions,accounts,categories,recurrences,primaryCurrency}:{transactions:Transaction[];accounts:MoneyAccount[];categories:MoneyCategory[];recurrences:MoneyRecurrence[];primaryCurrency:string}) {
  const [view,setView]=useState<"analysis"|"category"|"future">("analysis");
  ...
  const [reportCategoryExpanded,setReportCategoryExpanded]=useState<string|null>(null);
  ...
  const reportCategory=categories.find(category=>category.id===reportCategoryId)||null;
  const reportCategoryChildren=reportCategory&&!reportCategory.parentId?categories.filter(category=>category.parentId===reportCategory.id).map(category=>category.id):[];
  const categoryMatches=(transaction:Transaction)=>Boolean(reportCategory&&(transaction.categoryId===reportCategory.id||reportCategoryChildren.includes(transaction.categoryId||"")));
  const categoryPeriodRows=effective.filter(item=>monthKeys.some(key=>item.dateISO?.startsWith(key))&&categoryMatches(item));
  const categoryMovementRows=categoryPeriodRows.filter(item=>movement==="all"||movement==="income"&&(item.kind==="income"||item.kind==="refund")||movement==="expense"&&item.kind==="expense");
  const categoryIncome=categoryPeriodRows.filter(item=>item.kind==="income"||item.kind==="refund").reduce((sum,item)=>sum+converted(item.amount,item.accountId),0);
  const categoryExpense=categoryPeriodRows.filter(item=>item.kind==="expense").reduce((sum,item)=>sum+converted(item.amount,item.accountId),0);
  const categoryBalance=categoryIncome-categoryExpense;
  const categoryMonthly=monthKeys.map(key=>{const rows=categoryPeriodRows.filter(item=>item.dateISO?.startsWith(key));return {key,label:monthLabel(key),income:rows.filter(item=>item.kind==="income"||item.kind==="refund").reduce((sum,item)=>sum+converted(item.amount,item.accountId),0),expense:rows.filter(item=>item.kind==="expense").reduce((sum,item)=>sum+converted(item.amount,item.accountId),0)}});
  const categoryChartMaximum=Math.max(1,...categoryMonthly.flatMap(item=>movement==="income"?[item.income]:movement==="expense"?[item.expense]:[item.income,item.expense]));
  const reportCategoryRoots=categories.filter(category=>!category.parentId).sort((a,b)=>a.kind.localeCompare(b.kind)||a.name.localeCompare(b.name,"it"));
  ...
  return <section className="section-page report-explorer">
    <div className="report-main-switch report-three-switch"><button className={view==="analysis"?"active":""} onClick={()=>setView("analysis")}>Analisi</button><button className={view==="category"?"active":""} onClick={()=>setView("category")}>Per categoria</button><button className={view==="future"?"active":""} onClick={()=>setView("future")}>Futuro</button></div>
    {view==="analysis"?<>
      ...
      <div className="report-category-controls panel"><div><small>CATEGORIA O SOTTOCATEGORIA</small><button type="button" className="report-category-trigger" onClick={()=>setReportCategoryOpen(true)}>{reportCategory?(()=>{const visual=categoryVisual(reportCategory);const parent=reportCategory.parentId?categories.find(category=>category.id===reportCategory.parentId):null;return <><span style={{color:visual.color,background:`${visual.color}18`}}><AppIcon name={visual.icon} size={19}/></span><b>{parent?`${parent.name} › `:""}{reportCategory.name}</b><i>⌄</i></>})():<><span><AppIcon name="category" size={19}/></span><b>Seleziona categoria</b><i>⌄</i></>}</button></div><label>Periodo<select value={period} onChange={event=>setPeriod(Number(event.target.value) as 1|3|6|12)}><option value="1">Questo mese</option><option value="3">Ultimi 3 mesi</option><option value="6">Ultimi 6 mesi</option><option value="12">Ultimi 12 mesi</option></select></label><div className="movement-filter"><button className={movement==="all"?"active":""} onClick={()=>setMovement("all")}>Tutto</button><button className={movement==="income"?"active":""} onClick={()=>setMovement("income")}>Entrate</button><button className={movement==="expense"?"active":""} onClick={()=>setMovement("expense")}>Uscite</button></div></div>
      {reportCategory?<><div className="report-kpis category-report-kpis"><article><span>Entrate</span><b className="positive">{accountMoney(categoryIncome,primaryCurrency)}</b></article><article><span>Uscite</span><b>{accountMoney(-categoryExpense,primaryCurrency)}</b></article><article><span>Saldo</span><b className={categoryBalance>=0?"positive":""}>{accountMoney(categoryBalance,primaryCurrency)}</b></article><article><span>Transazioni</span><b>{categoryMovementRows.length}</b></article></div><article className="panel report-category-chart"><div className="panel-title"><div><h3>Andamento di {reportCategory.name}</h3><p>{period===1?"Questo mese":`Ultimi ${period} mesi`}</p></div><button type="button" className="outline" onClick={()=>setShowCategoryTransactions(value=>!value)}>{showCategoryTransactions?"Nascondi transazioni":`Vedi transazioni (${categoryMovementRows.length})`}</button></div><div className="bars interactive-bars">{categoryMonthly.map(item=><div key={item.key}>{movement!=="expense"&&<i className="report-income" title={accountMoney(item.income,primaryCurrency)} style={{height:`${item.income/categoryChartMaximum*100}%`}}/>}{movement!=="income"&&<i className="report-expense" title={accountMoney(item.expense,primaryCurrency)} style={{height:`${item.expense/categoryChartMaximum*100}%`}}/>}<span>{item.label.slice(0,3)}</span></div>)}</div></article>{showCategoryTransactions&&<article className="panel report-category-transactions"><div className="panel-title"><div><h3>Transazioni del periodo</h3><p>{categoryMovementRows.length} movimenti trovati</p></div></div>{[...categoryMovementRows].sort((a,b)=>(b.dateISO||"").localeCompare(a.dateISO||"")).map(item=>{const itemCategory=categories.find(category=>category.id===item.categoryId);const visual=itemCategory?categoryVisual(itemCategory):{icon:"transactions",color:"#777"};const account=accounts.find(value=>value.id===item.accountId);return <div className="report-transaction-row" key={item.id}><span style={{color:visual.color,background:`${visual.color}18`}}><AppIcon name={visual.icon} size={18}/></span><div><b>{itemCategory?.name||item.label}</b><small>{account?.name||item.account} · {item.date}</small>{item.notes&&<small>{item.notes}</small>}</div><strong className={item.kind==="income"||item.kind==="refund"?"positive":""}>{accountMoney(item.amount,account?.currency||primaryCurrency)}</strong></div>})}{!categoryMovementRows.length&&<div className="empty">Nessuna transazione per questa categoria nel periodo.</div>}</article>}</>:<article className="panel report-category-empty"><AppIcon name="category" size={28}/><h3>Seleziona una categoria</h3><p>Scegli una categoria o sottocategoria dal pannello visuale per creare il report.</p></article>}
      {reportCategoryOpen&&<div className="modal-backdrop report-category-picker-layer" onMouseDown={()=>setReportCategoryOpen(false)}><div className="category-picker report-category-picker" onMouseDown={event=>event.stopPropagation()}><div className="category-picker-title"><div><small>REPORT PER CATEGORIA</small><h3>Categoria e sottocategoria</h3></div><button type="button" onClick={()=>setReportCategoryOpen(false)}><AppIcon name="close" size={21}/></button></div><div className="category-tree">{reportCategoryRoots.map(root=>{const visual=categoryVisual(root);const children=categories.filter(category=>category.parentId===root.id).sort((a,b)=>a.name.localeCompare(b.name,"it"));const expanded=reportCategoryExpanded===root.id;return <div className="category-group" key={root.id}><button type="button" onClick={()=>{if(!children.length){setReportCategoryId(root.id);setReportCategoryOpen(false);setShowCategoryTransactions(false)}else setReportCategoryExpanded(expanded?null:root.id)}}><span className="real-icon" style={{color:visual.color,background:`${visual.color}18`}}><AppIcon name={visual.icon} size={16}/></span><b>{root.name}</b><small>{root.kind==="income"?"Entrata":"Uscita"}</small><i><AppIcon name={!children.length?"forward":expanded?"up":"down"} size={15}/></i></button>{expanded&&children.length>0&&<div className="subcategory-list"><button type="button" className="subcategory-choice report-root-choice" onClick={()=>{setReportCategoryId(root.id);setReportCategoryOpen(false);setShowCategoryTransactions(false)}}><span className="sub-symbol real-icon" style={{color:visual.color,background:`${visual.color}18`}}><AppIcon name={visual.icon} size={16}/></span><div><b>Tutta la categoria {root.name}</b></div></button>{children.map(child=>{const childVisual=categoryVisual(child);return <button type="button" className="subcategory-choice" key={child.id} onClick={()=>{setReportCategoryId(child.id);setReportCategoryOpen(false);setShowCategoryTransactions(false)}}><span className="sub-symbol real-icon" style={{color:childVisual.color,background:`${childVisual.color}18`}}><AppIcon name={childVisual.icon} size={16}/></span><div><b>{child.name}</b></div><i><AppIcon name="forward" size={14}/></i></button>})}</div>}</div>})}</div></div></div>}
    </>:<>
...
function TemplateManagementVisual({accounts,categories}:{accounts:MoneyAccount[];categories:MoneyCategory[]}){
  ...
  useEffect(()=>setTemplates(loadTransactionTemplates()),[]);
  const persist=(next:TransactionTemplate[])=>{try{saveTransactionTemplates(next);setTemplates(next);return true}catch(error){alert(error instanceof Error?`Impossibile salvare il modello: ${error.message}`:"Impossibile salvare il modello.");return false}};
...
function TransactionModal({ kind, close, add, accounts, cards, categories, preset = "normal", defaultAccount, cardId, initial, editing = false, refundSource }: { kind: ActionKind; close: () => void; add: (t: Transaction) => void | Promise<void>; accounts: MoneyAccount[]; cards: MoneyCard[]; categories: MoneyCategory[]; preset?: "normal" | "planned" | "subscription"; defaultAccount?: string; cardId?: string; initial?: Transaction; editing?: boolean; refundSource?: Transaction }) {
  ...
  const [amountPadTarget,setAmountPadTarget]=useState<"amount"|"received"|"voucher"|null>(null);
  const pickerTouchY=useRef<number|null>(null);
  const categoryLayerRef=useRef<HTMLDivElement|null>(null);
  const transactionModalRef=useRef<HTMLFormElement|null>(null);
  const [accountOpen, setAccountOpen] = useState(false);
  ...
  const [voucherCount, setVoucherCount] = useState(() => initial?.voucherCount!=null?String(initial.voucherCount):"");
  ...
  const openAmountPad=(target:"amount"|"received"|"voucher",event:React.PointerEvent<HTMLInputElement>)=>{if(typeof window!=="undefined"&&window.matchMedia("(max-width: 620px)").matches){event.preventDefault();event.currentTarget.blur();setAmountPadTarget(target)}};
  ...
  const pressAmountKey=(key:string)=>{if(amountPadTarget==="amount")setAmountValue(current=>{const next=nextAmountValue(current,key);if(isTransfer)setReceivedValue(amountInput(proposedReceived(next)));return next});else if(amountPadTarget==="received")setReceivedValue(current=>nextAmountValue(current,key));else if(amountPadTarget==="voucher")setVoucherCount(current=>key===","?current:nextAmountValue(current,key).replace(/\D/g,""))};
  const containCategoryTouch=(event:React.TouchEvent<HTMLDivElement>)=>{const currentY=event.touches[0]?.clientY??pickerTouchY.current;if(currentY==null)return;const previousY=pickerTouchY.current??currentY;pickerTouchY.current=currentY;const tree=(event.target as HTMLElement).closest(".category-tree") as HTMLElement|null;if(!tree){event.preventDefault();event.stopPropagation();return}const movingDown=currentY>previousY;const atTop=tree.scrollTop<=0;const atBottom=Math.ceil(tree.scrollTop+tree.clientHeight)>=tree.scrollHeight;if(tree.scrollHeight<=tree.clientHeight||(movingDown&&atTop)||(!movingDown&&atBottom))event.preventDefault();event.stopPropagation()};
  useEffect(()=>{if(!categoryOpen)return;const layer=categoryLayerRef.current;const modal=transactionModalRef.current;if(!layer||!modal)return;const modalScroll=modal.scrollTop;const keepModalStill=()=>{if(modal.scrollTop!==modalScroll)modal.scrollTop=modalScroll};let lastY=0;const start=(event:TouchEvent)=>{lastY=event.touches[0]?.clientY??0};const move=(event:TouchEvent)=>{const currentY=event.touches[0]?.clientY??lastY;const tree=(event.target as HTMLElement).closest(".category-tree") as HTMLElement|null;const movingDown=currentY>lastY;lastY=currentY;const canScroll=tree&&tree.scrollHeight>tree.clientHeight;const atTop=!tree||tree.scrollTop<=0;const atBottom=!tree||Math.ceil(tree.scrollTop+tree.clientHeight)>=tree.scrollHeight;if(!canScroll||(movingDown&&atTop)||(!movingDown&&atBottom))event.preventDefault();event.stopPropagation()};layer.addEventListener("touchstart",start,{passive:true});layer.addEventListener("touchmove",move,{passive:false});modal.addEventListener("scroll",keepModalStill,{passive:true});return()=>{layer.removeEventListener("touchstart",start);layer.removeEventListener("touchmove",move);modal.removeEventListener("scroll",keepModalStill)}},[categoryOpen]);
  ...
  return <div className="modal-backdrop"><form ref={transactionModalRef} className={`modal${categoryOpen?" category-lock-active":""}`} onSubmit={submit}>
    ...
    {isMealVoucher ? <div className="transaction-voucher-box">
      ...
      <label>Numero di buoni<input name="voucherCount" type="text" inputMode="none" value={voucherCount} onPointerDown={event=>openAmountPad("voucher",event)} onChange={e=>setVoucherCount(e.target.value.replace(/\D/g,""))} required/></label>
    ...
    {categoryOpen && <div ref={categoryLayerRef} className="picker-dim-layer keyboard-aware-picker" onMouseDown={()=>setCategoryOpen(false)} onWheel={event=>event.stopPropagation()} onTouchStart={event=>{pickerTouchY.current=event.touches[0]?.clientY??null}} onTouchMove={containCategoryTouch} onTouchEnd={()=>{pickerTouchY.current=null}}><div className="category-picker" onMouseDown={event=>event.stopPropagation()}>
    ...
    {amountPadTarget&&<div className="money-keypad-layer" onPointerDown={event=>{if(event.target===event.currentTarget)setAmountPadTarget(null)}}><div className="money-keypad"><div className="money-keypad-head"><span>{amountPadTarget==="voucher"?"BUONI":amountPadTarget==="received"?destinationAccount?.currency:amountCurrency}</span><strong>{amountPadTarget==="voucher"?(voucherCount||"0"):amountPadTarget==="received"?(receivedValue||"0,00"):(amountValue||"0,00")}</strong><button type="button" onPointerDown={event=>{event.preventDefault();pressAmountKey("backspace")}} aria-label="Cancella ultima cifra"><AppIcon name="back" size={22}/></button></div><div className="money-keypad-grid">{["1","2","3","4","5","6","7","8","9",",","0"].map(key=><button type="button" key={key} disabled={amountPadTarget==="voucher"&&key===","} onPointerDown={event=>{event.preventDefault();pressAmountKey(key)}}>{key}</button>)}<button type="button" className={`money-keypad-confirm ${kind}`} onPointerDown={event=>{event.preventDefault();event.stopPropagation()}} onPointerUp={event=>{event.preventDefault();event.stopPropagation();window.setTimeout(()=>setAmountPadTarget(null),100)}} onClick={event=>{event.preventDefault();event.stopPropagation()}} aria-label="Chiudi tastierino"><AppIcon name="check" size={28}/></button></div></div></div>}
  </form></div>
}
...
export default function Home() {
  ...
  const saveTransaction = async (transaction: Transaction) => {
    ...
    if(transaction.amount<0&&!transaction.planned&&category){
      ...
      const budget=budgets.find(item=>(item.categoryId===category.id||item.categoryId===category.parentId)&&item.month.startsWith(month));
      if(budget){
        const budgetCategoryIds=new Set([budget.categoryId,...categories.filter(child=>child.parentId===budget.categoryId).map(child=>child.id)]);
        const alreadySpent=netBudgetSpend(transactions.filter(item=>item.id!==transaction.id),budgetCategoryIds,month);
