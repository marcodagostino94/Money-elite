    ...
    <>
      {duePending.length > 0 && <section className="pending-confirmations panel">
        ...
        <div className="pending-items">
          {duePending.map(item=><div className="pending-item" key={item.id}><div className="pending-date"><b>{item.date.split(" ")[0]}</b><span>{item.date.split(" ")[1]}</span></div><div><b>{item.label}{item.notes?.trim()?` - ${item.notes.trim()}`:""}</b><span>{item.account}</span></div><strong>{money(item.amount)}</strong><button onClick={()=>void confirmTransaction(item)}>Conferma</button></div>)}
        </div>
      </section>}
        ...
        <article className="panel planned-panel">
          ...
          <div className="planned-grid">
            {futurePlanned.map(item=>{
              ...
              const title=item.kind==="transfer"?"Trasferimento tra conti":category?.name||"Pianificata";
              return <button className="planned-item" key={item.id} onClick={()=>setActive("Pianificate")}><div className="planned-date"><b>{date[0]}</b><span>{date[1]}</span></div><div><b>{title}{item.notes?.trim()?` - ${item.notes.trim()}`:""}</b><span>{account}</span></div><strong className={signedAmount>0?"positive":""}>{money(signedAmount)}</strong></button>;
            })}
...
function PlannedSection({recurrences,accounts,cards,categories,refresh,onEdit,onDuplicate}:{recurrences:MoneyRecurrence[];accounts:MoneyAccount[];cards:MoneyCard[];categories:MoneyCategory[];refresh:()=>Promise<void>;onEdit:(recurrence:MoneyRecurrence)=>void;onDuplicate:(recurrence:MoneyRecurrence)=>void}) {
  ...
  const labelFor=(item:MoneyRecurrence)=>{const category=categories.find(c=>c.id===item.categoryId);const title=item.kind==="transfer"?"Trasferimento tra conti":category?.name||"Pianificata";const note=item.notes?.trim();return note?`${title} - ${note}`:title};
...
function TransactionModal({ kind, close, add, accounts, cards, categories, preset = "normal", defaultAccount, cardId, initial, editing = false, refundSource }: { kind: ActionKind; close: () => void; add: (t: Transaction) => void | Promise<void>; accounts: MoneyAccount[]; cards: MoneyCard[]; categories: MoneyCategory[]; preset?: "normal" | "planned" | "subscription"; defaultAccount?: string; cardId?: string; initial?: Transaction; editing?: boolean; refundSource?: Transaction }) {
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}>
    ...
    </div> : <label>{isTransfer?"Importo inviato":"Valore"}<div className="amount-input"><span>{amountCurrency}</span><input name="amount" type="text" inputMode="decimal" required placeholder="0,00" value={amountValue} onChange={event=>{setAmountValue(event.target.value);if(isTransfer)setReceivedValue(amountInput(proposedReceived(event.target.value)))}}/></div></label>}
    ...
    {categoryOpen && <div className="picker-dim-layer" onMouseDown={()=>setCategoryOpen(false)}><div className="category-picker" onMouseDown={event=>event.stopPropagation()}>
